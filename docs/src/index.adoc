...
link:https://bintray.com/jmini/maven/download_file?file_path=fr%2Fjmini%2Fhtmlchecker%2Fhtmlchecker-cli%2F{project-version}%2Fhtmlchecker-cli-{project-version}-bin.zip[htmlchecker-cli-{project-version}-bin.zip].
...
[source,xml]
.Repository registration in the pom.xml example
----
include::../../site-example/pom.xml[tags="repo", indent="0"]
----
...
In order to build a release (push a tag to github, upload the binaries to bintray) just run:
